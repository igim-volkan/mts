...
export function EditLead() {
    ...
    const [formData, setFormData] = useState({
        ...
        companyName: '',
        sectors: [] as string[],
        lastContactDate: new Date().toISOString().slice(0, 16),
        ...
        if (id && leads.length > 0) {
            ...
            if (leadToEdit) {
                setFormData({
                    ...
                    sectors: leadToEdit.sectors,
                    lastContactDate: new Date(leadToEdit.lastContactDate).toISOString().slice(0, 16),
                    contactDirection: leadToEdit.contactDirection,
                    notes: leadToEdit.notes || '',
                    status: leadToEdit.status,
                });
            } else {
                // If lead is not found locally, ideally we would fetch it from DB directly,
                // or just navigate back. Let's redirect to leads list.
                navigate('/leads');
            }
        }
    }, [id, leads, navigate]);
        ...
        <div className="max-w-3xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <h1 className="text-3xl font-display font-bold">Müşteri Düzenle</h1>
                    <p className="text-slate-500 mt-1">Sistemdeki müşteri kaydını güncelleyin.</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-200 space-y-6">
                    <div className="border-b border-slate-100 pb-4">
                        <h3 className="text-xl font-semibold text-slate-800">Müşteri ve Firma Bilgileri</h3>
                        <p className="text-sm text-slate-500 mt-1">Sisteme ekli olan hedefin temel tanımlayıcı bilgileri.</p>
                    </div>
                    ...
                    <div className="space-y-1.5 focus-within:text-blue-600 transition-colors">
                        ...
                        <input
                            ...
                            className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all text-slate-900"
                        />
                    </div>
                    ...
                    <div className="space-y-3 pt-4 border-t border-slate-100">
                        <label className="text-sm font-medium text-slate-800">Sektörler (Çoklu Seçim)</label>
                ...
                <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-200 space-y-6">
                    <div className="border-b border-slate-100 pb-4">
                        <h3 className="text-xl font-semibold text-slate-800">Durum & Notlar</h3>
                        <p className="text-sm text-slate-500 mt-1">Müşterinin sistemdeki aşaması ve görüşme özetleri.</p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-1.5 focus-within:text-blue-600 transition-colors flex flex-col justify-end">
                            <label className="text-sm font-medium">İletişim Yönü</label>
                            <select
                                name="contactDirection"
                                value={formData.contactDirection}
                                onChange={handleChange}
                                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all text-slate-900 appearance-none"
                            >
                    ...
                    <div className="space-y-1.5 focus-within:text-blue-600 transition-colors">
                        ...
                        <textarea
                            ...
                            onChange={handleChange}
                            rows={5}
                            className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all resize-none text-slate-900"
                        />
                    </div>
                </div>
